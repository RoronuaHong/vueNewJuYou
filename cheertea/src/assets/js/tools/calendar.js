(function (wid, dcm) {
	...
	Calendar.prototype = {
		...
		renderCallbackArr: function (arr) {
            ...
            var docs = document.getElementById("months");

            //获取当前月
			var newMonth = new Date(arr[0].stamp).getMonth() + 1;

            //获取当前月份
            var monEle = document.querySelector("#container2TitleCenter");

            var monEles = monEle.innerHTML.substring(0, 1);
            ...
            for(var k = 0; k < arr.length; k++) {
                //判断是否为本月
                dateEles = new Date(arr[k].stamp);
                if (monEles == (dateEles.getMonth()) + 1) {
                    isTrue = false;
                    break;
                }
            }
            ...
            if(!isTrue) {
                docs.innerHTML = "本月免单日：" + monEles + "月" + dateEles.getDate() + "号";
            } else {
                docs.innerHTML = "本月免单日：暂无";
            }
			loop(0, arr.length, function (k) {
				// if (!$class(_this.container + '-item-' + arr[k].stamp)[0]) {
                 //    alert($class(_this.container + '-item-' + arr[k].stamp))
